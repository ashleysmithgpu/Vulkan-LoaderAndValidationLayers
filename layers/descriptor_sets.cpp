...
: some_update_(false), full_update_(false), set_(set), p_layout_(layout), buffer_map_(buffer_map), memory_map_(memory_map), buffer_view_map_(buffer_view_map), sampler_map_(sampler_map), image_view_map_(image_view_map), image_map_(image_map), image_to_swapchain_map_(image_to_swapchain_map), swapchain_map_(swapchain_map) {
...
//
bool cvdescriptorset::DescriptorSet::ValidateDrawState(const std::unordered_set<uint32_t> &bindings, const std::vector<uint32_t> &dynamic_offsets, std::string* error) const {
        ...
        if (descriptors_.at(start_idx)->IsImmutableSampler()) {
            ...
            for (uint32_t i = start_idx; i <= end_idx; ++i) {
                if (!descriptors_.at(i)->updated) {
                    std::stringstream error_str;
                    error_str << "Descriptor in binding #" << binding << " at global descriptor index " << i << " is being used in draw but has not been updated.";
                    *error = error_str.str();
                    return false;
                } else {
                    if (GeneralBuffer == descriptors_.at(i)->GetClass()) {
                        ...
                        if (buffer_node == buffer_map_->end()) {
                            std::stringstream error_str;
                            error_str << "Descriptor in binding #" << binding << " at global descriptor index " << i << " references invalid buffer " << buffer << ".";
                            *error = error_str.str();
                            return false;
                        } else {
                            ...
                            if (mem_entry == memory_map_->end()) {
                                std::stringstream error_str;
                                error_str << "Descriptor in binding #" << binding << " at global descriptor index " << i << " uses buffer " << buffer << " that references invalid memory " << buffer_node->second.mem << ".";
                                *error = error_str.str();
                                return false;
                            }
                        }
                        if (descriptors_.at(i)->IsDynamic()) {
                            ...
                            if ((VK_WHOLE_SIZE == range) && ((dyn_offset + desc_offset) > buffer_size)) {
                                std::stringstream error_str;
                                error_str << "Dynamic descriptor in binding #" << binding << " at global descriptor index " << i << " uses buffer " << buffer << " with update range of VK_WHOLE_SIZE has dynamic offset " << dyn_offset << " combined with offset " << desc_offset << " that oversteps the buffer size of " << buffer_size << ".";
                                *error = error_str.str();
                                return false;
                            } else if ((dyn_offset + desc_offset + range) > buffer_size) {
                                std::stringstream error_str;
                                error_str << "Dynamic descriptor in binding #" << binding << " at global descriptor index " << i << " uses buffer " << buffer << " with dynamic offset " << dyn_offset << " combined with offset " << desc_offset << " and range " << range << " that oversteps the buffer size of " << buffer_size << ".";
                                *error = error_str.str();
                                return false;
                            }
                        }
...
bool cvdescriptorset::DescriptorSet::VerifyUpdateConsistency(uint32_t current_binding, uint32_t binding_remaining, uint32_t update_count, const char* type, std::string* error_msg) const {
    ...
    while (update_count > binding_remaining) { // While our updates overstep current binding
        ...
        if (!p_layout_->IsNextBindingConsistent(current_binding++)) {
            std::stringstream error_str;
            error_str << "Attempting " << type << " descriptor set " << set_ << " binding #" << orig_binding << " with #" << update_count << " descriptors being updated but this update oversteps the bounds of this binding and then next binding is not consistent with current binding so this update is invalid.";
            *error_msg = error_str.str();
            return false;
        }
        ...
        *error_msg = error_str.str();
        ...
        for (auto di=0; di<update->descriptorCount; ++di) {
            if (!descriptors_[start_idx+di]->WriteUpdate(update, di, error_msg)) {
                std::stringstream error_str;
                error_str << "Write update to descriptor at global index " << start_idx+di << " within set " << set_ << " binding #" << update->dstBinding << " failed with error message: " << error_msg->c_str();
                *error_msg = error_str.str();
                return false;
            }
            ++num_updates;
    ...
    if (!p_layout_->HasBinding(update->dstBinding)) {
        ...
        *error = error_str.str();
        ...
        if ((src_start_idx + update->descriptorCount) > src_set->GetTotalDescriptorCount()) {
            ...
            std::stringstream error_str;
            error_str << "Attempting copy update from descriptorSet " << update->srcSet << " binding#" << update->srcBinding << " with offset index of " << src_set->GetGlobalStartIndexFromBinding(update->srcBinding) << " plus update array offset of " << update->srcArrayElement << " and update of " << update->descriptorCount << "descriptors oversteps total number of descriptors in set: " << src_set->GetTotalDescriptorCount() << ".";
            *error = error_str.str();
            return false;
        }
        ...
        if ((dst_start_idx + update->descriptorCount) > p_layout_->GetTotalDescriptorCount()) {
            ...
            std::stringstream error_str;
            error_str << "Attempting copy update to descriptorSet " << set_ << " binding#" << update->dstBinding << " with offset index of " << p_layout_->GetGlobalStartIndexFromBinding(update->dstBinding) << " plus update array offset of " << update->dstArrayElement << " and update of " << update->descriptorCount << "descriptors oversteps total number of descriptors in set: " << p_layout_->GetTotalDescriptorCount() << ".";
            *error = error_str.str();
            return false;
        }
        ...
        if ((!VerifyUpdateConsistency(update->srcBinding, src_binding_remaining, update->descriptorCount, "copy update from", error)) || (!VerifyUpdateConsistency(update->dstBinding, dst_binding_remaining, update->descriptorCount, "copy update to", error))) {
            return false;
        }
